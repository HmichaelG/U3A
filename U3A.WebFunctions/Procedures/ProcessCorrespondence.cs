...
namespace U3A.WebFunctions.Procedures
{
    public static class ProcessCorrespondence
    {
            ...
            string tenantConnectionString, U3AFunctionOptions options)
        {
            ...
            ProFormaReportFactory reportFactory = new(tenant);
            ...
            using TenantDbContext dbcT = new(tenantConnectionString);
            ...
            else
            {
                ...
                foreach (SendMail sm in await BusinessRule.GetMultiCampusMailAsync(dbcT, tenant.Identifier!))
                {
            ...
            foreach (SendMail sm in mailItems)
            {
                ...
                {
                    case "Cash Receipt":
                        Receipt? receipt = await dbc.Receipt.IgnoreQueryFilters()
                                            .Include(x => x.Person)
                    ...
                    case "Participant Enrolment":
                        ...
                        {
                            sm.Status = "Enrollment not found";
                        }
                    ...
                    case "Leader Report":
                        ...
                        {
                            Person? leader = await dbc.Person.IgnoreQueryFilters()
                                                .Where(x => !x.IsDeleted && x.ID == sm.PersonID).FirstOrDefaultAsync();
                            int todayID = (int)today.DayOfWeek;
                            int classOnDayID = -1;
                            ...
                            else
                            {
                                ...
                                if (course != null)
                                {
                                    ...
                                    enrolments.AddRange(mcEnrolments.Where(x => x.CourseID == course.ID && x.ClassID == null
                                                                          && x.TermID == sm.TermID));
                                    foreach (Class? c in await dbc.Class.Where(x => x.CourseID == course.ID).OrderBy(x => x.OnDayID).ToListAsync())
                                    {
            ...
            foreach (KeyValuePair<Guid, string> kvp in enrolmentResults)
            {
                foreach (SendMail? sm in await dbc.SendMail.IgnoreQueryFilters().Where(x => x.PersonID == kvp.Key).ToListAsync())
                {
                    sm.Status = kvp.Value;
                }
                foreach (MultiCampusSendMail? sm in await dbcT.MultiCampusSendMail.Where(x => x.PersonID == kvp.Key).ToListAsync())
                {
                    sm.Status = kvp.Value;
                }
            }
            ...
            int postalCount = reportFactory.PostalReports.Count;
            if (postalCount > 0)
            {
            ...
            dbcT.RemoveRange(dbcT.MultiCampusSendMail.AsEnumerable()
                .Where(x => (today - x.CreatedOn.GetValueOrDefault()).Days > 30));
            int deleted = dbc.ChangeTracker.Entries().Where(x => x.State == EntityState.Deleted).Count();
