using U3A.Services.APIClient.Converters;
using U3A.Services.APIClient.Exceptions;
using U3A.Services.APIClient.Utility;
using System;
...
public abstract class APIClientBase
{
    ...
    private static readonly string _agent = "U3A-functions 1.x (" +
          typeof(APIClient).AssemblyQualifiedName + ")";
    ...
    public APIClientBase(string apiBaseUri = "")
    {
        baseUri = new Uri(apiBaseUri);
    }

    protected string getBaseUri(string apiBaseUri)
    {
        string result = apiBaseUri;
        if (result == "")
        {
            apiBaseUri = (constants.IS_DEVELOPMENT) ? "https://localhost/7071/api/" : "https://u3a-functions.azurewebsites.net/api/";
        }
        return result;
    }
        ...
        HttpMethod verb, TRequestBody message = null) where TRequestBody : class
    {
        ...
        if (!JsonExtensions.TryDeserializeObject<APIClientResponse>(body, out APIClientResponse error))
        {
        ...
        {
            case 422:
                error.Status = APIClientStatus.UserError;
                break;
            case 500:
                error.Status = APIClientStatus.ServerError;
                break;
            default:
                error.Status = APIClientStatus.Unknown;
                break;
        }

        throw new APIClientValidationException(error);
    }
